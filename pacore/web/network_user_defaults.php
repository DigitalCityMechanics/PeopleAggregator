...
if (($action == 'save' || $action == 'store_as_defaults') && !$error && !$imported_defaults) {//if data is posted
  ...
  if ((isset($_POST['relate_me'])) && ($_POST['relate_me'] == 1))  { //if add me as a friend check box is checked
  ...
  //check whether supplied login names exist
    ...
    foreach($user_names as $name) {
      $n = trim($name);
      if (!empty($n)) {
        if (!in_array($n, $found_name)) {//check for repeated login names
          try {
            $related_user_id_array = User::map_logins_to_ids($n);//getting login_name=>user_id array
            foreach ( $related_user_id_array as $key=>$values) {
              $related_user_id = $values; //getting user_id
            }
            if( Network::member_exists((int)PA::$network_info->network_id,(int)$related_user_id)) {//if user_id exist for this network

...
<?php
  ...
  if (!$error && ($do == 'approve')) {
    Network::approve_suggestion((int)$_GET['cid']);
    header("Location: ". PA::$url ."/network_manage_suggestions.php?msg_id=1002");exit;
  } else if (!$error && ($do == 'deny')) {
    Network::disapprove_suggestion((int)$_GET['cid']);
    header("Location: ". PA::$url ."/network_manage_suggestions.php?msg_id=1003");exit;
  }
  if (!empty($_POST['submit'])) {
    if (empty($_POST['cid'])) {
      header("Location: ". PA::$url ."/network_manage_suggestions.php?msg_id=1007");exit;
    }
    switch( $action ) {
      case 'deny':
        try {
          ...
          for ($counter = 0; $counter < $total_content; $counter++) {            
            Network::disapprove_suggestion((int)$content_array[$counter]);
          }
          header("Location: ". PA::$url ."/network_manage_suggestions.php?msg_id=1003");exit;
        }
        catch ( PAException $e ) {
      ...
      case 'approve':
        try {
          ...
          $total_content = count($content_array);
          for ($counter = 0; $counter < $total_content; $counter++) {            
            Network::approve_suggestion((int)$content_array[$counter]);
          }
          header("Location: ". PA::$url ."/network_manage_suggestions.php?msg_id=1002");exit;
        }
        catch ( PAException $e ) {
      ...
      break;
      default:
        header("Location: ". PA::$url ."/network_manage_suggestions.php?msg_id=1006");exit;  
    }
  }
  ...
  function setup_module($column, $module, $obj) {
    global $paging;
    switch($module){    
      case 'NetworkManageSuggestionsModule':

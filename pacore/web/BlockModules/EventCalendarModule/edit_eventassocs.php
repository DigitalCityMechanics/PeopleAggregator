...
<h2><?= __("This event appears on") ?></h2>
    ...
    foreach ($assocs as $n=>$assoc) {
    	...
    	$appears_in[] = $assoc->assoc_target_type . ":" 
        . $assoc->assoc_target_id;
      echo "<li>";
      echo __("Calendar for")." "
        . $assoc->assoc_target_type . " " 
        . $assoc->assoc_target_name;
      // add remove link if we are allowed to
      $may_remove = false;
      // user is creator/owner
      if ($assoc->user_id == PA::$login_user->user_id)  $may_remove = true;
      // user is target
      if ($assoc->assoc_target_type == 'user'
        && $assoc->assoc_target_id == PA::$login_user->user_id) $may_remove = true;
      // user is admin
      if ($assoc->assoc_target_type == 'group') {
	      $is_member = Group::get_user_type((int)PA::$login_user->user_id, (int)$assoc->assoc_target_id);
	      if ($is_member == 'owner') $may_remove = true;
      }
      ...
      if ($may_remove) {
        ...
        && $assoc->assoc_target_id == PA::$login_user->user_id)) {
          ...
          echo " <input type=\"submit\" name=\"remove_assoc_"
        . $assoc->assoc_id
        . "\" value=\"".__("Remove")."\" />";
        }
      }

...
//echo "Action: " . $this->action . "Module: " . $this->module . "clname: " . $this->class_name; die();
  ...
  private function build_list_of_items($items) {
    ...
    foreach($items as $item) {
      ...
      switch($results[$cnt]['media_type']) {
          ...
          break;
          case 'audio': $default_pic = uihelper_resize_mk_img(null, 86, 92, 'images/default_audio.png', "", RESIZE_CROP);
          break;
          case 'video': $default_pic = uihelper_resize_mk_img(null, 86, 92, 'images/default_video.png', "", RESIZE_CROP);
          break;
          default:
  ...
  function handleRequest($request_method, $request_data) {
    if(!empty($this->action) && ($this->module == $this->class_name)) { // only if action and target module defined

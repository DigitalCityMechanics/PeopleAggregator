...
<?php for($i = 0; $i < $count; $i++) {?>   
  <li>
    <?php  // for Images and their hyperlinks
       if ( $links[$i]['type'] == 7) {
         $image = "<img src='".PA::$theme_url."/images/sb-70X50.jpg' width='70px' height='38px' border='0' alt='SB'/>";
    ...
    <?php if (PA::$page_uid == $curr_user_id && empty($_GET['gid'])) { 
              if($links[$i]['type'] == 7) {
                  ...
                  $link_for_editing = PA::$url . "/edit_media.php?uid=".PA::$page_uid."&amp;cid=".$links[$i]['content_id']."&amp;type=video";
              }
    ?>         
     <p class="choose">
       ...
       <a href="#"><img src="<?php echo PA::$theme_url;?>/images/16_delete.gif" alt="" height="16" width="16" border="0"  title="Delete" onclick="edit_delete_media('<?=$links[$i]['content_id']?>','delete','image_upload2'); return false;"></a>
    </p>

...
class Navigation {
    ...
    function make_links() {
        ...
        if (PA::$config->enable_network_spawning) {
            if (PermissionsHandler::can_user(PA::$login_uid, array('permissions' => 'create_networks'))) {
        ...
        $level_2 = array(
            'people' => array(
                'caption' => __(PA::$people_noun),
                'icon' => __('fa fa-user'),
                'url' => $this->base_url . PA_ROUTE_PEOPLES_PAGE
            ),
            'groups' => array(
                'caption' => __(PA::$group_noun_plural),
                'icon' => __('fa fa-group'),
        ...
        if (!empty(PA::$config->useTypedGroups)) {
            $level_2 = $level_2 + array(
                'directory' => array(
                    'caption' => __('Directory'),
                    'icon' => __('fa fa-bookmark'),
        ...
        //    'shop' => array(
        //        'caption' => __('Shop'),
        //        'icon' => __('fa fa-square-o'),
        //        'url' => $this->base_url . PA_ROUTE_SHOPS . "/network_id=" . $this->network_info->network_id,
        //    ),
        //);
        $level_2 = $level_2 + array(
            'forum' => array(
        ...
        if (PermissionsHandler::can_user(PA::$login_uid, array('permissions' => 'manage_widgets'))) {
            $user_children = $user_children + array(
        ...
        if (PermissionsHandler::can_user(PA::$login_uid, array('permissions' => 'manage_messages'))) {
            $user_children = $user_children + array(
        ...
        if (PermissionsHandler::can_user(PA::$login_uid, array('permissions' => 'manage_media'))) {
            $user_children = $user_children + array(
        ...
        if (PermissionsHandler::can_user(PA::$login_uid, array('permissions' => 'manage_events'))) {
            $user_children = $user_children + array(
        ...
        $user_children = $user_children + array(
            'my_friends' => array('caption' => __('My Friends'),
                'icon' => __('fa fa-users'),
                'url' => $this->base_url . "/user_all_members.php?view_type=in_relations&amp;uid=$uid"
            ),
        );

        if (PermissionsHandler::can_user(PA::$login_uid, array('permissions' => 'manage_forums'))) {
            $user_children = $user_children + array(
        ...
        if (PermissionsHandler::can_user(PA::$login_uid, array('permissions' => 'manage_themes'))) {
            $user_children = $user_children + array(
            ...
            if ($group_may_post) {
                ...
                $group_specific = array(
                    'create_post' => array(
                        ...
                        'url' => PA::$url . '/content_blog.php?ccid=' . $gid
                    )
                        ) + $group_specific;
            }
        ...
        $network_default = array(
            'user_by_admin' => array('caption' => __('Create User'),
        ...
        switch ($this->current_page) {
            ...
            case PAGE_USER_PRIVATE:
                if (!PA::$page_uid && !PA::$login_uid) {
                    throw new Exception("", "Invalid page access");
                }

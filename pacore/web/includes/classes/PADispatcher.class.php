 ...
 */
 ...
 define('F_NOT_FOUND', "Requested file not found on this server.");
 define('F_NO_PERMIS', "You do not have permission to download this file.");
 define('F_NOT_ALLOW', "This file type is not allowed on this server");
 define('F_STR_ERROR', "PADownloadManager is unable to create file output stream.");
 define('F_NO_ROUTE',  "PADispatcher is unable to resolve requested route.");
 ...
 class PADispatcher {
     ...
     if($prr_succes) {
       if(isset($project_routes) && (count($project_routes) > 0)) {
         ...
         foreach($project_routes as $pr_key => $pr_val) {
           $p_route = preg_split("/[\(]+|[\[]+/", $pr_key);  // split optional route params
           foreach($rkeys as $key) {                         // check is route overwritten by a project rewr. rule
              if(false !== strpos($key, $p_route[0])) {      // yes - route is owerwritted
                unset($this->routes[$key]);                  // delete old route
                break;
              }
           }
           $this->routes[$pr_key] = $pr_val;                 // add route rewritting rule
         }
       }
     }
   }
   ...
   private function bootApp($script) {
     ...
     default_exception(); // register default exception handler
     ...
     if(PA::$ssl_security_on) {
       if($this->dispatcher_scheme != $this->routing_scheme) {
         ...
         header("Location: " . $this->routing_scheme . "://". $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI']);// "$path_pref/$file_name" . $path_info . $guery_str);
         exit;
       }
     }
...
if(!function_exists('getShadowedPath')) {  // global function
 function getShadowedPath($file_path) {
   ...
   } else if(file_exists(PA_PROJECT_CORE_DIR . DIRECTORY_SEPARATOR . $file_path)) {
...
if(!function_exists('out_error404')) {  // global function
...
if(!function_exists('out_error_message')) {  // global function

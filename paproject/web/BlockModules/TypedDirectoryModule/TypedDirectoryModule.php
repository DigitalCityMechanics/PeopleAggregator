  ...
  function initializeModule($request_method, $request_data) {
    ...
    $this->groupCount = array();
  	...
  	if ($this->directoryType) {
		...
		$this->sortFields = array(
			array('name' => 'name', 'label'  => __('Name')),
			array('name' => 'created', 'label'  => __('Date Created')),
			array('name' => 'updated', 'label'  => __('Last Updated'))
		);
		foreach ($this->profilefields as $i=>$field) {
			if (!empty($field['sort'])) {
				$this->sortFields[] = array(
		...
		$this->totalGroupCount = TypedGroupentity::get_total_count();

		// load list of entities
		$this->typedGroupEntities =
			TypedGroupentity::get_all_entities($this->sort_by, $this->page, $this->show);

		$this->Paging['count'] = $this->totalGroupCount;
		$Pagination = new Pagination();
		$Pagination->setPaging($this->Paging);

...
class Pagination 
{
  ...
  public $separator = ' ';
  public $str_backward = 'Previous';
  public $str_forward = 'Next';
  public $max_links = 5;
  ...
  public function getPageLinks() 
  {
    ...
    if ($curr_pages <= $all_pages && $curr_pages >= 0) {
      ...
      if($all_pages >= 1) {
        ...
        $navi_string .= ($curr_pages > 1) ? "<a class=\"previous_page\" href=\"".$this->normalize_query($this->_pageDetails['url']."?".$var."=".$backward.$this->rebuildQueryString($var)."\"".$this->_pageDetails['extra']).' rel="prev">'.$this->str_backward.'</a> ' : '<span class="previous_page disabled">'.$this->str_backward.'</span>';
        ...
        //for($a = $start + 1; $a <= $end; $a++){
          for($a = $start; $a <= $end; $a++){
            ...
            if ($theNext != $curr_pages) {
              $navi_string .= "<a href=\"".$this->normalize_query($this->_pageDetails['url']."?".$var."=".$theNext.$this->rebuildQueryString($var)."\"".$this->_pageDetails['extra']).' rel="'.($a < $curr_pages ? 'prev' : 'next').'">';
              $navi_string .= $a.'</a>'."\n";
            } else {
              $navi_string .= '<em>'.$a.'</em>'."\n";
            }
        ...
        $navi_string .= ($curr_pages < $all_pages) ? "<a class=\"next_page\" href=\"".$this->normalize_query($this->_pageDetails['url']."?".$var."=".$forward.$this->rebuildQueryString($var)."\"".$this->_pageDetails['extra'])." rel=\"next\">".$this->str_forward."</a>" : '<span class="next_page disabled">'.$this->str_forward.'</span>';
      }
    }

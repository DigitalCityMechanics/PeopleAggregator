...
class Network {
  ...
  */
  static function get_network_members( $nid, $params = NULL, $conditions = NULL) {
    ...
    $sql = "SELECT NU.user_id, U.avatar, U.avatar_width, U.avatar_height, U.avatar_small, U.avatar_small_width, U.avatar_small_height, U.login_name, U.first_name, U.last_name, U.email, U.created, U.last_login, U.is_active,CONCAT_WS(' ', U.first_name, U.last_name) AS display_name";
    if ( @$params['cnt']==TRUE ) {
   ...
   if(!empty($params['extra_condition'])) {
    ...
    if($res->numRows()) {
      while ($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
        ...
        $users_data[$i]['avatar'] = $row->avatar;
        $users_data[$i]['avatar_width'] = $row->avatar_width;
        $users_data[$i]['avatar_height'] = $row->avatar_height;
        $users_data[$i]['avatar_small'] = $row->avatar_small;
        $users_data[$i]['avatar_small_width'] = $row->avatar_width;
        $users_data[$i]['avatar_small_height'] = $row->avatar_height;
        ...
        $users_data[$i]['last_login'] = $row->last_login;
